...
export async function generateLesson(opts) {
  ...
  if (threeD || physics || tutor) {
    const err = new Error(
      "--3d/--physics/--tutor are being repackaged as @faraday-academy/* addon packages and are temporarily unavailable. Scaffold a 2D lesson for now.",
    );
    err.exitCode = 2;
    throw err;
  }
  ...
  // 5. provenance (VCS-tracked identity; records the runtime line it was created with)
  await fs.mkdir(path.join(targetDir, ".faraday"), { recursive: true });
  await fs.writeFile(
    path.join(targetDir, ".faraday", "provenance.json"),
    JSON.stringify(
      { lessonId: uuid(), createdWith: "faraday@0.1.0", template: "starter@0.1.0", runtime: "@faraday-academy/runtime@0.1.0", name: packageName },
      null,
      2,
    ) + "\n",
  );
